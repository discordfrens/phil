...
export const permissionHandler = (permissions: PermissionResolvable[]) => {
  return {
    bot: (client_member: GuildMember) => {
      if(client_member.permissions.has("Administrator") || client_member.permissions.has(permissions)) return true;
      return;
    },
...
export const getUser = (userId: string, ctx: Message) => {
  if(!userId) return;
  return ctx.mentions.members.first() || ctx.guild.members.cache.find(f => f.id === userId || f.user.tag.toLowerCase() === userId.toLowerCase() || f.user.username.toLowerCase() === userId.toLowerCase()) || ctx.guild.members.cache.find(f => f.nickname ? f.nickname.toLowerCase() === userId.toLowerCase() : false)
}
...
export const isAdminOrMod = (user: GuildMember, guild: Guild) => {
  ...
  return user.permissions.has("Administrator") || user.permissions.has("ManageGuild") || user.roles.highest.position > c.roles.highest.position
}